...
int main(){
  ...
  do{
      ...
      x0=x1;
      x1=x;
      porcentual = abs(x1-x0)/x0*100; //ERROR PARCIAL 2 usando error absoluto porcentual se haría una división entre 0: ( (valor_real - valor_inicial) / valor_real ), por eso utilizo el iterativo porcentual.
      cout << "Error porcentual = " << porcentual << endl;
      i++;
  }while(abs(porcentual) > error); //ERROR PARCIAL 2
